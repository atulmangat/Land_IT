...
public class LanderCanvas extends GCanvas {
    ...
    private static final float GRAVITY_ACCELERATION = .3f;

    // upward acceleration due to thrusters
    private static final float THRUST_ACCELERATION = -.6f;
    ...
    public void init() {
        ...
        rocket.setLocation( getWidth()/2 ; 0)
        add(rocket);
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            Toast.(getApplicationContext(),"Your toast message.",Toast.LENGTH_SHORT).show();
            isThrusting = true;
